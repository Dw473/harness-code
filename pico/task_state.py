...
@dataclass
class TaskState:
    run_id: str  # 本次 run 目录名，如 run_20260623-...
    task_id: str  # 任务 id，如 task_20260623-...
    user_request: str  # 用户这条消息
    status: str = STATUS_RUNNING  # running / completed / stopped / failed
    tool_steps: int = 0  # 真正执行了的工具次数
    attempts: int = 0  # 模型被调用了几轮（含 retry）
    last_tool: str = ""  # 上一个工具名
    stop_reason: str = ""  # 为什么停
    final_answer: str = ""  # 最终答案（若有）
    checkpoint_id: str = ""  # 结束时关联的最新 checkpoint
    resume_status: str = ""  # 本轮开始时的 resume 判定
