...
from .config import load_pico_env, provider_env
...
def _configured_secret_names(args):
    #获取api_key的环境变量名
    #DEFAULT_SECRET_ENV_NAMES是默认的api_key的环境变量名
    #args.secret_env_names是用户传入的api_key的环境变量名
    #extra_names是全局环境变量名
...
def build_agent(args):
    ...
    # Provider keys live with the pico install (.env in pico-main), not the target workspace.
    load_pico_env()
