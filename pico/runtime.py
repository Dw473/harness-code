...
class Pico:
    def __init__(
        ...
        # --- 模型与工作区 ---
        self.model_client = model_client  # 模型后端（DeepSeek / OpenAI / Anthropic / Ollama）
        self.workspace = workspace  # 工作区快照：git 状态、分支、README 片段等
        self.root = Path(workspace.repo_root)  # 仓库根目录；文件工具与 path 沙箱的锚点

        # --- 会话与持久化 ---
        self.session_store = session_store  # 读写 .pico/sessions/<id>.json，跨 ask 保留对话
        self.approval_policy = approval_policy  # 高风险工具审批：ask / auto / never
        self.max_steps = max_steps  # 单次 ask 最多几次工具步
        self.max_new_tokens = max_new_tokens  # 每轮模型输出 token 上限
        self.depth = depth  # 当前嵌套深度（根 agent=0，delegate 子 agent 递增）
        self.max_depth = max_depth  # 最大嵌套深度；子 agent 达到上限后不再暴露 delegate
        self.read_only = read_only  # 只读模式；为 True 时禁止写文件、跑 shell 等
        self.shell_env_allowlist = tuple(shell_env_allowlist or DEFAULT_SHELL_ENV_ALLOWLIST)  # shell 允许透传的环境变量
        self.secret_env_names = {str(name).upper() for name in (secret_env_names or ())}  # trace/report 脱敏的 env 名
        self.feature_flags = dict(DEFAULT_FEATURE_FLAGS)  # memory、context_reduction、prompt_cache 等开关
        if feature_flags:
            self.feature_flags.update({str(key): bool(value) for key, value in feature_flags.items()})
        self.allowed_tools = self._normalize_allowed_tools(allowed_tools)  # 工具白名单；None 表示全部可用
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".pico" / "runs")  # 单次 ask 的 trace/report
        self.session = session or {  # 当前会话：history、memory、checkpoints 等；可 from_session 恢复
        ...
        self._ensure_session_shape()  # 补全 checkpoints、runtime_identity 等缺失字段
        self.memory = memorylib.LayeredMemory(  # 工作记忆 + durable memory 读写
            self.session.setdefault("memory", memorylib.default_memory_state()),
            workspace_root=self.root,
        )
        self.session["memory"] = self.memory.to_dict()  # 与 LayeredMemory 同步，便于落盘

        # --- 工具与 prompt ---
        self.tools = self._apply_tool_allowlist(self.build_tools())  # {name: schema, risky, run}
        self.tool_executor = ToolExecutor(self)  # 校验、审批、执行工具的总闸口
        self.prefix_state = self.build_prefix()  # PromptPrefix：文本 + hash + fingerprint
        self.prefix = self.prefix_state.text  # 系统规则 + 工具说明 + workspace，供 ContextManager 使用
        self.context_manager = ContextManager(self)  # 按预算组装完整 prompt

        # --- checkpoint / resume ---
        self.resume_state = self.evaluate_resume_state()  # full-valid / partial-stale / no-checkpoint 等
        self.session_path = self.session_store.save(self.session)  # 当前 session JSON 路径

        # --- 当前 run 运行时状态（ask 期间才有值）---
        self.current_task_state = None  # 正在进行的 TaskState
        self.current_run_dir = None  # .pico/runs/<run_id>/

        # --- 上一轮 metadata 缓存（供 report / trace）---
        self.last_prompt_metadata = {}  # 最后一轮 prompt 组装元数据
        self.last_completion_metadata = {}  # 最后一轮模型 usage/cache 统计
        self.last_durable_promotions = []  # 最近一次成功写入的长期记忆
        self.last_durable_rejections = []  # 最近一次被拒绝的长期记忆
        self.last_durable_superseded = []  # 最近一次被新事实替换的旧记忆
        self._last_tool_result_metadata = {}  # 最后一次工具执行的 tool_status 等
        self._last_prefix_refresh = {  # 上次 refresh_prefix 是否变更 workspace/prefix
    ...
    def reject_durable_reason(self, note_text):
        #持久记忆（durable memory）入库前的过滤器：判断某条「长期事实」该不该写进 .pico/memory/
    ...
    def extract_durable_promotions(self, user_message, final_answer):
        ...
        promotions = []
        rejections = []
        #通过匹配类型+过滤规则，返回promotions, rejections两个列表
    ...
    def repeated_tool_call(self, name, args):
        ...
        # 这里提前挡掉最简单的这种循环。
        #若这 2 次的 name 和 args 都和当前这次相同，判定为重复
    ...
    def tool_example(self, name):
        #返回该工具的 正确调用示例，参数校验失败时附在错误信息里，教模型怎么写。
    ...
    def spawn_delegate(self, args):
        #父 agent 通过 delegate 工具派出一个受限的子 Pico，只做调查，把结论文本还给父 agent。
    ...
    def parse(raw):
        ...
        #解析出 tool：交给 run_tool() 统一安全边界执行工具
        #解析出 final：直接结束对话，返回结果给用户
        #解析出 retry：把报错提示塞回 prompt，让模型重新输出规范内容
    ...
    def parse_xml_tool(raw):
        #输出和 JSON 格式<tool>完全相同的字典结构，上层parse()不用区分两种工具格式，统一交给run_tool()执行。
    ...
    def parse_attrs(text):
        #parse_attrs 解析 XML 风格 <tool> 标签上的 key="value" 属性，供 parse_xml_tool 拼成统一的 {name, args}。
        attrs = {}
        for match in re.finditer(r"""([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?:"([^"]*)"|'([^']*)')""", text):
            attrs[match.group(1)] = match.group(2) if match.group(2) is not None else match.group(3)
        return attrs

    #两个都是 从文本里取出 <tag>...</tag> 中间内容 的小工具
    @staticmethod
