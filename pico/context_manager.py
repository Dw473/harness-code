...
DEFAULT_TOTAL_BUDGET = 12000  # 整份 prompt 字符上限
...
CURRENT_REQUEST_SECTION = "current_request"  # 当前用户请求段；永不裁剪
RELEVANT_MEMORY_LIMIT = 3  # relevant_memory 最多检索几条 note


def _tail_clip(text, limit):
    """从文本头部截断到 limit 字符，超出部分用 ... 代替。"""
    text = str(text)
    if limit <= 0:
        return ""
    if len(text) <= limit:
...
@dataclass
class SectionRender:
    """单个 prompt 段的渲染结果：原始文本、预算、裁剪后文本及细节。"""
...
class ContextManager:
    """按预算把 prefix、memory、history 等拼成完整 prompt。"""
    ...
    def __init__(
        ...
        self.agent = agent  # Pico 实例；提供 prefix、memory、session 等
    ...
    def build(self, user_message):
        ...
        # 读取 feature_flags，决定是否启用 memory / 相关记忆检索 / 预算压缩
        memory_enabled = True
        ...
        # 关闭 context_reduction 时，不做预算裁剪，直接全量渲染
        ...
        # 按 section 预算渲染；超 total_budget 则循环压缩
    ...
    def _render_sections_without_reduction(self, section_texts, selected_notes=None):
        """不裁剪预算，全量渲染各 section（context_reduction 关闭时用）。"""
    ...
    def _compute_section_floors(self):
        """计算各 section 压缩下限：默认为预算的 1/4，可被 section_floors 覆盖。"""
        floors = {
    ...
    def _render_sections(self, section_texts, budgets, selected_notes=None):
        """按给定 budgets 渲染所有 section，返回 {section: SectionRender}。"""
        rendered = {}
    ...
    def _render_relevant_memory(self, selected_notes, budget):
        """渲染 relevant_memory 段：检索到的 notes 列表，每条平分预算。"""
    ...
    def _per_note_budget(self, budget, note_count, header):
        """计算 relevant_memory 段里每条 note 可用的字符预算。"""
        if note_count <= 0:
            return 0
    ...
    def _render_history_section(self, budget):
        """渲染 history 段：优先保留最近 6 条，更早条目压缩或折叠。"""
        history = list(getattr(self.agent, "session", {}).get("history", []))
    ...
    def _compressed_history_entries(self, history, recent_start):
        """把 session history 转成可渲染条目；旧 read_file 折叠，旧 tool 压成摘要。"""
        entries = []
    ...
    def _reusable_file_summary(self, path):
        """从 working memory 取某文件的短摘要，供旧 read_file 条目折叠时使用。"""
    ...
    def _summarize_old_tool_item(self, item):
        """把较早的 tool 结果压成一行摘要（run_shell 取命令+前几行输出）。"""
    ...
    def _raw_history_text(self, history):
        """history 未压缩时的完整 Transcript 文本（用于 raw_chars 统计）。"""
        if not history:
            return "Transcript:\n- empty"
        lines = []
        for item in history:
    ...
    def _render_history_item(self, item, line_limit):
        """把单条 history 项渲染为 1~2 行文本（tool 带 args 前缀 + 内容）。"""
        if item["role"] == "tool":
    ...
    def _assemble_prompt(self, rendered):
        """按固定顺序拼接各 section 为最终 prompt 字符串。"""
    ...
    def _metadata(self, prompt, rendered, budgets, reduction_log, selected_notes, user_message, section_texts):
        """生成 prompt 组装元数据，供 trace/report 解释各段长度与压缩情况。"""
        section_metadata = {}
