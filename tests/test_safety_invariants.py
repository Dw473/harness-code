...
def test_cli_build_agent_loads_pico_env_secrets_before_redaction_setup(tmp_path):
    class DummyModelClient:
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs
    ...
    with patch.dict(os.environ, {}, clear=True), patch(
        "pico.config.find_pico_project_root",
        return_value=tmp_path,
    ), patch("pico.cli.AnthropicCompatibleModelClient", DummyModelClient):
