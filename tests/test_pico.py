...
def test_build_agent_uses_pico_env_provider_when_cli_omitted(tmp_path):
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch("pico.config.find_pico_project_root", return_value=tmp_path), patch(
            "pico.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch(
...
def test_build_agent_ignores_workspace_env_file(tmp_path):
    workspace = tmp_path / "workspace"
    pico_root = tmp_path / "pico-root"
    workspace.mkdir()
    pico_root.mkdir()
    (workspace / "README.md").write_text("demo\n", encoding="utf-8")
    (workspace / ".env").write_text("PICO_DEEPSEEK_API_KEY=sk-from-workspace\n", encoding="utf-8")
    (pico_root / ".env").write_text(
        "\n".join(
            [
                "PICO_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "PICO_DEEPSEEK_API_KEY=sk-from-pico",
    ...
    args = pico_pkg.build_arg_parser().parse_args(["--cwd", str(workspace), "--provider", "deepseek"])

    with patch.dict(os.environ, {}, clear=True):
        with patch("pico.config.find_pico_project_root", return_value=pico_root), patch(
            "pico.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch(
    ...
    assert mock_anthropic.call_args.kwargs["api_key"] == "sk-from-pico"


def test_build_agent_prefers_cli_provider_over_pico_env_provider(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "PICO_PROVIDER=openai",
